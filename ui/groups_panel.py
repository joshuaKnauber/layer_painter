...
def group_panel_poll(context):
    ...
    if interface_utils.base_poll(context) and ntree and ntree.bl_idname == "ShaderNodeTree":
        if ntree == context.active_object.active_material.node_tree:
            if ntree.nodes.active and hasattr(ntree.nodes.active, "node_tree") and ntree.nodes.active.node_tree:
                return ntree.nodes.active.node_tree.uid == ""
    return False
...
class LP_PT_Groups(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not group_stngs.is_mask:
            row = layout.row()
            row.scale_y = 1.5
            row.prop(group_stngs, "is_mask", toggle=True,
                     text="Turn into mask", icon="CHECKMARK")

        elif group_stngs.is_mask:
            row = layout.row()
            row.scale_y = 1.5
            row.prop(group_stngs, "is_mask", toggle=True,
                     text="Disable mask", icon="PANEL_CLOSE", invert_checkbox=True)

            row = layout.row()
            row.scale_y = 1.2
            if context.space_data.edit_tree == context.active_object.active_material.node_tree:
                row.operator("node.group_edit", text="Edit group",
                             icon="GREASEPENCIL")
            else:
                row.operator("node.group_edit",
                             text="Stop editing", icon="GREASEPENCIL")
...
class LP_PT_GroupPreview(bpy.types.Panel):
    ...
    def draw_header(self, context):
        self.layout.label(
            text=f"{context.space_data.node_tree.nodes.active.node_tree.name} UI Preview")
