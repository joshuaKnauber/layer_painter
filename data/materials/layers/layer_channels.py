...
def __connect_layer_output(layer, channel, out):
    ...
    if above and not above.node: raise f"Couldn't find layer node for '{above.name}'. Delete layer to proceed."
...
def __connect_layer_input(layer, channel, inp):
    ...
    if below and not below.node: raise f"Couldn't find layer node for '{below.name}'. Delete layer to proceed."
...
def __remove_orphan_channels(layer):
    ...
    for i in range(len(layer.node.node_tree.inputs)-1, -1, -1):
