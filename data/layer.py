...
class LP_LayerProperties(bpy.types.PropertyGroup):
    ...
    def __add_channel_endpoints(self, channel):
        ...
        inp, group_inp = group_utils.add_input(
            self.node, channel.inp.bl_rna.identifier, channel.name)
        inp.default_value = channel.inp.default_value

        _, group_out = group_utils.add_output(
            self.node, channel.inp.bl_rna.identifier, channel.name)
    ...
    def __add_channel_opacity(self, mix):
        ...
        self.layer_ntree.links.new(
            self.layer_ntree.nodes[constants.OPAC_NAME].outputs[0], opacity.inputs[2])
    ...
    def __remove_channel(self, mix, uid):
        ...
        node_utils.remove_connected_left(
            self.layer_ntree, mix.inputs[2].links[0].from_node)
        ...
        self.layer_ntree.inputs.remove(
            self.layer_ntree.inputs[self.get_channel_input_index(uid)])
        self.layer_ntree.outputs.remove(
            self.layer_ntree.outputs[self.get_channel_output_index(uid)])
    ...
    def __update_channel_socket_names(self, changed_channel):
        ...
        inp_index, out_index = self.get_channel_endpoint_indices(
            changed_channel.uid)
    ...
    def cycle_channel_data_type(self, channel_uid):
        ...
        if data_type == "COL":
            ...
            self.layer_ntree.links.new(
                tex.outputs[1], self.get_channel_tex_alpha_socket(channel_uid))
            self.update_texture_mapping()

        elif data_type == "TEX":
            value_node = self.setup_fill_value_node(
                self.mat.lp.channel_by_uid(channel_uid))
    ...
    def update_texture_mapping(self, context=None):
        ...
        for channel in self.mat.lp.channels:
            if self.get_channel_data_type(channel.uid) == "TEX":
                tex, mapp, coords = self.__get_channel_texture_nodes(
                    channel.uid)
                ...
                if self.tex_coords == "UV":
                    self.layer_ntree.links.new(
                        coords.outputs["UV"], mapp.inputs[0])
